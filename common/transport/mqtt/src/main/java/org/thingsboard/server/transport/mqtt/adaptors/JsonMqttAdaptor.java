...
public class JsonMqttAdaptor implements MqttTransportAdaptor {
    ...
    @Override
    public TransportProtos.PostTelemetryMsg convertToPostCustomize(MqttDeviceAwareSessionContext ctx, String inbound){
            return JsonConverter.convertToCustomizeProto(new JsonParser().parse(inbound));
        //return JsonConverter.convertToCustomizeProto(inbound);
    }
    ...
    public static JsonElement validateJsonPayload(UUID sessionId, ByteBuf payloadData) throws AdaptorException {
        String payload = validatePayload(sessionId, payloadData, false);//直到这里payload都还只是pooledSliceByteBuf（），我理解成device端传过来的json型数据流，我们需要用这个函数把它转成string
        try {
    ...
    private static String validatePayload(UUID sessionId, ByteBuf payloadData, boolean isEmptyPayloadAllowed) throws AdaptorException {
        String payload = payloadData.toString(UTF8);////直到这里payload都还只是bytebuf，即pooledSliceByteBuf（），我理解成device端传过来的json型数据流，然后进行格式转换成tb认识的UTF8
        if (payload == null) {
