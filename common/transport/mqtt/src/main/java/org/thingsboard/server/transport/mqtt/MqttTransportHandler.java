...
//import static io.netty.handler.codec.mqtt.MqttMessageType.CUSTOMIZE;
...
public class MqttTransportHandler extends ChannelInboundHandlerAdapter implements GenericFutureListener<Future<? super Void>>, SessionMsgListener {
    ...
    //public static final MqttMessageType CONNECT=null;
    ...
    private void processMqttMsg(ChannelHandlerContext ctx, MqttMessage msg) {
        ...
        switch (msg.fixedHeader().messageType()) {
            case CONNECT://先处理连接
                ...
                break;
            //MqttMessageType CUSTOMIZE = null;
            /*case CUSTOMIZE://想自定义协议
                processPublish(ctx, (MqttPublishMessage) msg);//MqttPublishMessage也可以自定义成其他，但是需要反编译更改，这里的自定义暂时和publish一样，只是改了个名字
                break;            */                             //如果需要自定义，只要更改里面相应的函数就可以了
            case PUBLISH://然后处理发布
    ...
    //把xml转换成json
	public static String xml2jsonString(String xml) throws JSONException, IOException {
   ...
   /* public static JSONObject convertXmlToJson(String xml) throws IOException {
    ...
    private void processDevicePublish(ChannelHandlerContext ctx, MqttPublishMessage mqttMsg, String topicName, int msgId) {
        try {
            ...
            }else if (topicName.equals(MqttTopics.DEVICE_CUSTOMIZE_TOPIC)) {
                //byte[] bbb = new byte[mqttMsg.payload().capacity()]; //bytebuf转换成byte
                final Charset UTF8 = Charset.forName("UTF-8");
                String bbb=mqttMsg.payload().toString(UTF8);//把bytebuf转换成string
                //JSONObject req=xml2JSON(bbb.getBytes());//byte转换成json
                String req1=xml2jsonString(bbb);//把xml转换成json，把json转换成string
                 //mqttMsg.payload()=Unpooled.wrappedBuffer(req1);//将byte转换成bytebuf
                TransportProtos.PostTelemetryMsg postTelemetryMsg = adaptor.convertToPostCustomize(deviceSessionCtx, req1);//理论上可以去掉这句，只要将req1转换成PostTelemetryMsg格式就行了
                //TransportProtos.PostTelemetryMsg postTelemetryMsg = adaptor.convertToPostCustomize(deviceSessionCtx, bbb);
                transportService.process(sessionInfo, postTelemetryMsg, getPubAckCallback(ctx, msgId, postTelemetryMsg));
            }
        } /*catch (AdaptorException | JDOMException | IOException e) {
            log.warn("[{}] Failed to process publish msg [{}][{}]", sessionId, topicName, msgId, e);
            log.info("[{}] Closing current session due to invalid publish msg [{}][{}]", sessionId, topicName, msgId);
            ctx.close();
        }*/
        catch (AdaptorException | IOException e) {
            log.warn("[{}] Failed to process publish msg [{}][{}]", sessionId, topicName, msgId, e);
            log.info("[{}] Closing current session due to invalid publish msg [{}][{}]", sessionId, topicName, msgId);
            ctx.close();
        }
    ...
    private void processCustomize(ChannelHandlerContext ctx, MqttPublishMessage mqttMsg) {//和publish一样
        if (!checkConnected(ctx, mqttMsg)) {
            return;
        }
        String topicName = mqttMsg.variableHeader().topicName();
        int msgId = mqttMsg.variableHeader().packetId();//
        log.trace("[{}][{}] Processing Customize msg [{}][{}]!", sessionId, deviceSessionCtx.getDeviceId(), topicName, msgId);//可变头？

        if (topicName.startsWith(MqttTopics.BASE_GATEWAY_API_TOPIC)) {//因为目前自定义的协议做的事情暂时和publish一样，所以除了函数名，所有调用的函数暂时不定义，只是证明可以更改
            if (gatewaySessionHandler != null) {
                handleGatewayPublishMsg(topicName, msgId, mqttMsg);
            }
        } else {
...
//这里的ctx应该就是设备端里面的topic，msg就是设备端的message
    private void processAuthTokenConnect(ChannelHandlerContext ctx, MqttConnectMessage msg) {
        ...
        if (StringUtils.isEmpty(userName)) {
            ...
            ctx.close();
        } else { //取出userName,构造protobuf的类(方便传输与解析)，交给transportService处理。
            transportService.process(ValidateDeviceTokenRequestMsg.newBuilder().setToken(userName).build(),//成功调用onSuccess的Consumer
                    new TransportServiceCallback<ValidateDeviceCredentialsResponseMsg>() {
                        @Override
                        public void onSuccess(ValidateDeviceCredentialsResponseMsg msg) {
                            onValidateDeviceResponse(msg, ctx);//这里topic和message交换了位置
                        }
