...
public class RestClient implements ClientHttpRequestInterceptor {
   ...
   /* public Device assignDeviceUser(CustomerId userId, DeviceId deviceId) {
        return restTemplate.postForEntity(baseURL + "/api/user/{user}/device/{deviceId}", null, Device.class,
                userId.toString(), deviceId.toString()).getBody();
    }*/
    ...
    public Optional<Device> assignDeviceToUser(String UserId, String deviceId) {
        try {
            ResponseEntity<Device> device = restTemplate.postForEntity(baseURL + "/api/user/{UserId}/device/{deviceId}", null, Device.class, UserId, deviceId);
            return Optional.ofNullable(device.getBody());
        } catch (HttpClientErrorException exception) {
            if (exception.getStatusCode() == HttpStatus.NOT_FOUND) {
                return Optional.empty();
