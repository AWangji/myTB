...
//这里貌似是客户用户权限下的，查询方式，原本的是显示出所有客户的设备，但是自从把设备拿给用户私有以后，就需要改变显示的部分，只能显示该用户拥有的设备，这样的话定义的方法也要自定义一些。
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
...
//这部分是tenant中点击分配按钮的api，可以点击后进入输入搜索条件，然后调用deviceController里面的assignDeviceToCustomer分配相应设备给搜索出来的customerid
    @PreAuthorize("hasAuthority('TENANT_ADMIN')")
    ...
    public TextPageData<Customer> getCustomers(@RequestParam int limit,
                                               @RequestParam(required = false) String textSearch,//textSearch搜索条件
    ...
    //需要定义一个增加按钮，在customer账户中登录后，点击该按钮后映射到这个api，然后可以显示所有可购买的设备，需要修改搜索目的->变成搜索设备id，
    //然后分配的对象要可以直接get该账户下的customerid，然后选择后点击确认购买（这部分位于deviceController里面的assignDeviceToCustomer1），可以将改设备分配给自己
        /*@PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/customers1", params = {"limit"}, method = RequestMethod.GET)
    @ResponseBody
    public TextPageData<Device> getCustomers1(@RequestParam int limit,
