...
public class DeviceController extends BaseController {
    ...
    public Device saveDevice(@RequestBody Device device,
        ...
        try {
            ...
            Operation operation = device.getId() == null ? Operation.CREATE : Operation.WRITE;//创造设备id？
...
//在tenant账户中分配设备给点击的客户是这里
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/customer/{customerId}/device/{deviceId}", method = RequestMethod.POST)
    @ResponseBody
    public Device assignDeviceToCustomer(@PathVariable("customerId") String strCustomerId,
                                         @PathVariable(DEVICE_ID) String strDeviceId) throws ThingsboardException {//作为租户管理员身份，主动点击设备分配，
        checkParameter("customerId", strCustomerId);          //设备分配的时候需要选择设备，可是选择给已经添加好的客户，所以只能先添加客户再调用这个函数
        checkParameter(DEVICE_ID, strDeviceId);                                          //调用设备id和客户id两个参数，把相应设备分配给相应客户
        try {                                                        //貌似根本不用管是具体确定了某个设备，然后选择客户分配给他，还是说先确定客户然后去选择设备进行分配，调用的时候都会自动搞定
            ...
            Device savedDevice = checkNotNull(deviceService.assignDeviceToCustomer(getCurrentUser().getTenantId(), deviceId, customerId));//只是个检查

            logEntityAction(deviceId, savedDevice,
                    savedDevice.getCustomerId(),
                    ActionType.ASSIGNED_TO_CUSTOMER, null, strDeviceId, strCustomerId, customer.getName());//这句才是分配，推入规则引擎？
    ...
    public TextPageData<Device> getTenantDevices(
        ...
        try {
            ...
            if(textSearch==""){
                ...
                return checkNotNull(deviceService.findDevicesByTenantId(tenantId, pageLink));//看起来时checknull，但其实返回的是正宗的搜索到的结果
            }
        } catch (Exception e) {
            throw handleException(e);
        }
    }
    //自定义，在custmer_user账户中，点击增加设备后应当访问的api，会选择性的显示
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/customer/addDevices", params = {"limit"}, method = RequestMethod.GET)//自定义一个api可以不显示所有的租户设备
    @ResponseBody
    public TextPageData<Device> getCustomeruserDevices(
            @RequestParam int limit,
            @RequestParam(required = false) String textSearch,
            @RequestParam(required = false) String idOffset,
            @RequestParam(required = false) String textOffset) throws ThingsboardException {
        try {
            ...
            if(textSearch==""){
                ...
                return checkNotNull(deviceService.findDevicesByTenantId(tenantId, pageLink));//这里还要重新定义查找方式的函数，
            }
        } catch (Exception e) {
            throw handleException(e);
        }
...
//customer账户查询设备的部分,显示出该customer目前所拥有的设备
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    ...
    //自定义的在customer账户中添加自分配的功能，需要定义一个确认购买按钮，类似于tenant账户中改的分配
    //点击该按钮后映射到这个api，然后通过该api成功将设备分配给该customer账户自己（需要改写）
   /* @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/customer/{customerId}/devices/buy", params = {"limit"},  method = RequestMethod.POST)
    @ResponseBody
    public Device assignDeviceToCustomer1(@PathVariable("customerId") String strCustomerId,
                                         @PathVariable(DEVICE_ID) String strDeviceId) throws ThingsboardException {//作为租户管理员身份，主动点击设备分配，
        checkParameter("customerId", strCustomerId);          //设备分配的时候需要选择设备，可是选择给已经添加好的客户，所以只能先添加客户再调用这个函数
        checkParameter(DEVICE_ID, strDeviceId);                                          //调用设备id和客户id两个参数，把相应设备分配给相应客户
        try {
